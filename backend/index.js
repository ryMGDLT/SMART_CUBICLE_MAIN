...
app.post('/users/:userId/upload-profile-image', upload.single('profileImage'), (req, res) => {
    const userId = req.params.userId;
    const newFileName = req.file.filename;

    // Delete the old image file (if it exists)
    const oldFiles = fs.readdirSync(path.join(__dirname, "uploads/profile-images"))
    ...
    const serverUrl = process.env.BACKEND_URL || "http://192.168.113.78:5000";
    res.json({ profileImage: `${serverUrl}/uploads/profile-images/${newFileName}` });
});
// Routes
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`Backend server accessible on the network at http://${getIPAddress()}:${PORT}`);
});
